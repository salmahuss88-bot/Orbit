...
const Sidebar = () => {
    ...
    return (
      ...
      `}</style>
            ...
            <aside className="w-72 h-screen bg-white border-r border-gray-100 flex flex-col py-10 overflow-y-auto hide-scrollbar">
                ...
                <div className="flex flex-col items-center mb-12 px-8 text-center">
                    ...
                    <h1 className="text-[18px] font-bold text-[#3294C6] leading-tight">
                        Orbit
                    </h1>
                            ...
                            return (
                                <li key={item.name} className="w-full relative">
                                    <button
                                        ...
                                        className="flex items-center px-10 py-3 w-full relative outline-none group"
                                    >
                                        <div className={`absolute inset-0 bg-[#f4f9ff] transition-opacity duration-300 z-0
                      ${isActive ? 'opacity-100' : 'opacity-0'}`}
                                        />

                                        <div className={`absolute left-0 top-0 bottom-0 w-[6px] rounded-r-full z-20 
                      transition-all duration-300 ease-in-out
                      ${isActive
                                                ? 'translate-x-0 opacity-100 bg-gradient-to-b from-[#14ADD6] to-[#384295]'
                                                : '-translate-x-full opacity-0'}`}
                                        />
                                        <div className="flex items-center gap-4 z-10 w-full">
                                            <div className={`flex flex-row items-center gap-4 transition-all duration-300 ${isActive ? 'active-gradient-state' : 'text-black'}`}>

                                                {/* حجم الأيقونة */}
                                                <span className="text-[22px] flex items-center justify-center shrink-0">
                                                    {item.icon}
                                                </span>
